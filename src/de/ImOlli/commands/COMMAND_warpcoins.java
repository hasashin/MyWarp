...
public class COMMAND_warpcoins implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            p.sendMessage(MyWarp.getPrefix() + MessageManager.getMessage("MyWarp.warpcoins.showcoins").replaceAll("%amount%", WarpCostsManager.getWarpCoins(p).toString()).replaceAll("%currency%", WarpCostsManager.getCurrency()));
            return true;
        }
        ...
        if (args.length == 2) {
            if (args[1].equalsIgnoreCase("get")) {
                ...
                p.sendMessage(MyWarp.getPrefix() + MessageManager.getMessage("MyWarp.warpcoins.get").replaceAll("%name%", p2.getName()).replaceAll("%amount%", WarpCostsManager.getWarpCoins(p2).toString()).replaceAll("%currency%", WarpCostsManager.getCurrency()));
                return true;
            } else {
                p.sendMessage(MyWarp.getPrefix() + MessageManager.getMessage("MyWarp.cmd.error").replaceAll("%cmd%", "/warpcoins <add|get|remove> <player> <amount>"));
                return true;
